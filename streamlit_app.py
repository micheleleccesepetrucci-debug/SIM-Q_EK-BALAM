...
# Crear pestañas para organizar la suite de herramientas
tab_simulator, tab_report, tab_docs = st.tabs([
    "📊 Simulador Interactivo", 
    "📝 Generador de Reporte Técnico", 
    "📘 Criterios y Soporte Técnico"
])
...
# --- PESTAÑA 3: CRITERIOS Y SOPORTE TÉCNICO ---
with tab_docs:
    st.markdown("""
    <div class="panel-card">
        <h3>📘 Fundamento Metodológico y Criterios de Evaluación</h3>
        <p>Documento de soporte técnico referente al paso de la matriz clásica a la nueva matriz semicuantitativa de criticidad 3x3 ajustada según API RP 2SIM.</p>
    </div>
    """, unsafe_allow_html=True)
    
    docs_path = "documento_tecnico_soporte.md"
    if os.path.exists(docs_path):
        with open(docs_path, "r", encoding="utf-8") as f:
            md_content = f.read()
        st.markdown(md_content)
    else:
        st.warning("No se encontró el archivo documento_tecnico_soporte.md en el servidor.")
